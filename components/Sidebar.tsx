...
function AppLogo({ url, name }: { url: string | null; name: string }) {
  const initials = (name || 'PDM').substring(0, 2).toUpperCase()
  if (url) {
    return (
  ...
  return (
    <aside className="hidden lg:flex flex-col fixed left-0 top-0 h-full w-52 bg-[#0a0a0a] z-50 border-r border-white/5">
      <div className="flex items-center gap-2.5 px-4 py-4 border-b border-white/5">
        <AppLogo url={logoUrl} name={siteName} />
        <div>
          <p className="text-white font-black text-sm leading-none tracking-tight">{siteName.split(' ').slice(0,2).join(' ')}</p>
          ...
          return (
            <Link key={href} href={href}
              className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-all
                ${active ? 'bg-[#1a6b3a] text-white' : 'text-white/50 hover:text-white hover:bg-white/5'}`}>
