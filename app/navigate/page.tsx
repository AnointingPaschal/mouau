...
function NavigateContent() {
  ...
  // 'checking' = initial, no overlay shown yet
  ...
  // Returns coords on success, null on failure
  const doGetPos = useCallback((silent=false): Promise<{lat:number;lng:number}|null> =>
    new Promise(resolve => {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        err => { console.warn('GPS:',err.message); if(!silent) setLocState('denied'); resolve(null) },
        {enableHighAccuracy:true, timeout:10000, maximumAge:0}
      )
    })
  ,[])

  // On mount: check localStorage FIRST — skips overlay for returning users
  useEffect(()=>{
    const cached = localStorage.getItem(LOC_KEY)==='1'
    if(cached){
      // Already granted before — skip overlay, fetch silently
      setLocState('granted')
      doGetPos(true)
      return
    }
      ...
      navigator.permissions.query({name:'geolocation' as PermissionName})
        .then(result=>{
          if(result.state==='granted'){
            setLocState('granted'); localStorage.setItem(LOC_KEY,'1'); doGetPos(true)
          } else if(result.state==='denied'){
            setLocState('denied')
          } else {
            setLocState('requesting'); doGetPos(false)
          }
          result.onchange=()=>{
            if(result.state==='granted'){ setLocState('granted'); localStorage.setItem(LOC_KEY,'1'); doGetPos(true) }
            else if(result.state==='denied') setLocState('denied')
          }
        })
        .catch(()=>{ setLocState('requesting'); doGetPos(false) })
    } else {
      setLocState('requesting'); doGetPos(false)
    }
  },[doGetPos])
  ...
  // Only show overlay once permission state is known (not 'checking')
  ...
  return (
    <AppShell>
      ...
      <div className={`relative flex flex-col h-[calc(100vh-104px)] lg:h-[calc(100vh-60px)] transition-all duration-300 ${blur?'blur-md pointer-events-none select-none brightness-95':''}`}>
        ...
        <div className="bg-white border-b border-[#e8e8e8] px-3 py-3 space-y-2 z-20 relative">
            ...
            <div className={`flex items-center border rounded-xl bg-white shadow-sm transition-colors ${fromFocus?'border-[#1a6b3a]':'border-[#e8e8e8]'}`}>
              ...
              {locState==='granted'&&userLat&&!fromLoading&&(
                <button onClick={()=>{ setFromText(`${userLat!.toFixed(6)},${userLng!.toFixed(6)}`); setFromSugg([]) }}
                  className="px-2 text-[#1a6b3a] flex-shrink-0"><LocateFixed className="w-4 h-4"/></button>
              )}
          ...
          <button onClick={getDirections} disabled={loadingDir}
            ...
            {loadingDir
              ? <><Loader2 className="w-3.5 h-3.5 animate-spin"/>{gettingLoc?'Getting your location...':'Finding route...'}</>
        ...
        <div className="flex-1 relative">
          ...
          {/* Campus chips at map bottom when no directions */}
          {!dirResult&&(
                  ...
                  <div className="overflow-y-auto flex-1">
                    {steps.length===0?(
                      <div className="px-4 py-5 text-center">
                        <p className="text-xs text-[#aaa] mb-3">Route shown on map above.</p>
          ...
          {dirResult?._noSteps&&(
            <div className="absolute bottom-4 left-3 right-3 z-10">
              <div className="bg-white rounded-2xl shadow-xl border border-[#e8e8e8] p-4 flex items-center gap-3">
                ...
                <button onClick={openInApp} className="bg-[#1a6b3a] text-white rounded-xl px-3 py-2 text-[10px] font-semibold flex items-center gap-1 flex-shrink-0">
      ...
      {/* ── Overlay — rendered OUTSIDE blurred container ── */}
      {showOverlay&&(
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-[#e8e8e8] p-6 mx-4 max-w-xs w-full text-center animate-slide-up">
            {locState==='requesting'?(
              ...
              <>
                ...
                <button onClick={retryLocation} className="btn-primary w-full flex items-center justify-center gap-1.5 mb-2">
