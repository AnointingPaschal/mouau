...
function Avatar({ name, hasPwa }: { name: string; hasPwa: boolean }) {
  ...
  return (
    <div className="relative flex-shrink-0">
      ...
      {hasPwa && (
        <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-[#1a6b3a] rounded-full flex items-center justify-center shadow-sm border border-white"
          title="PWA Installed">
...
function StudentCard({ student, hasPwa }: { student: Student; hasPwa: boolean }) {
  ...
  return (
    <div className="bg-white border border-[#e8e8e8] rounded-2xl p-3.5 flex items-start gap-3 hover:border-[#1a6b3a]/30 transition-colors">
      <Avatar name={student.name} hasPwa={hasPwa} />
...
export default function StudentsPage() {
  ...
  const [pwaInstalled, setPwaInstalled] = useState<Set<string>>(new Set())
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/students', { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(d => {
        setStudents(d.data || [])
        setPwaInstalled(new Set(d.pwaInstalled || []))
        setLoading(false)
      })
      .catch(() => setLoading(false))
  }, [token])

  const pwaStudents = useMemo(
    () => students.filter(s => pwaInstalled.has(s.id_number)),
    [students, pwaInstalled]
  )

  const baseList = tab === 'pwa' ? pwaStudents : students
  ...
  const pwaPercent = students.length
    ? Math.round((pwaInstalled.size / students.length) * 100)
    : 0
      ...
      <div className="p-4 w-full pb-24 space-y-4 max-w-2xl">
        ...
        <div className="grid grid-cols-3 gap-2.5">
          ...
          <div className="card p-3 text-center">
            ...
            <p className="font-black text-[#1a6b3a] text-lg leading-none">{pwaInstalled.size}</p>
        ...
        {!loading && students.length > 0 && (
          <div className="card p-3">
            <div className="flex items-center justify-between text-[10px] font-semibold mb-1.5">
              ...
              <span className="text-[#1a6b3a]">{pwaInstalled.size} / {students.length}</span>
            </div>
        ...
        <div className="flex gap-1.5 bg-[#f5f5f3] p-1 rounded-xl">
          {([
            ...
            { id: 'pwa', label: '📲 PWA Installed', count: pwaInstalled.size },
          ] as const).map(t => (
            <button key={t.id}
              onClick={() => setTab(t.id)}
          ...
          <div className="space-y-2">
            {filtered.map(s => (
              <StudentCard
                key={s.id}
                student={s}
                hasPwa={pwaInstalled.has(s.id_number)}
              />
            ))}
          </div>
        )}
