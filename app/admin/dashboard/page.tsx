...
import {
  BookOpen, Users, Bell, MapPin, Brain, LayoutDashboard,
  ChevronRight, MessageSquare, Calendar, Flag
} from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  const [stats, setStats] = useState({
    students: 0, materials: 0, posts: 0, locations: 0, training: 0,
    events: 0, pendingReports: 0
  })
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch('/api/admin/forum?view=posts', { headers: h }).then(r => r.json()),
      fetch('/api/admin/forum?view=reports', { headers: h }).then(r => r.json()),
    ]).then(([s, m, l, ai, ev, fp, fr]) => {
      setStats({
        ...
        events: ev.data?.filter((e: any) => e.active).length || 0,
        posts: fp.data?.length || 0,
        pendingReports: fr.data?.filter((r: any) => r.status === 'pending').length || 0,
      })
    })
  }, [token])

  const cards = [
    { href: '/admin/events',        label: 'Events',         desc: `${stats.events} active`,             icon: Calendar,       color: 'bg-amber-50 text-amber-600' },
    { href: '/admin/forum',         label: 'Forum',          desc: `${stats.posts} posts${stats.pendingReports > 0 ? ` · ${stats.pendingReports} reports` : ''}`, icon: MessageSquare, color: stats.pendingReports > 0 ? 'bg-red-50 text-red-600' : 'bg-[#1a6b3a]/10 text-[#1a6b3a]' },
    ...
    <AdminShell>
      <div className="p-5 lg:p-8 max-w-4xl">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          {[
            { label: 'Students', value: stats.students, color: '#1a6b3a' },
            { label: 'Materials', value: stats.materials, color: '#2563eb' },
            { label: 'Forum Posts', value: stats.posts, color: '#7c3aed' },
            { label: 'Pending Reports', value: stats.pendingReports, color: stats.pendingReports > 0 ? '#e11d48' : '#6b6b6b' },
        ...
        {stats.pendingReports > 0 && (
          <Link href="/admin/forum" className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl p-4 mb-4 hover:bg-red-100 transition-colors">
            <div className="w-9 h-9 bg-red-100 rounded-xl flex items-center justify-center flex-shrink-0">
              <Flag className="w-4 h-4 text-red-500" />
            </div>
            <div className="flex-1">
              <p className="font-bold text-red-700 text-sm">{stats.pendingReports} Reported Post{stats.pendingReports > 1 ? 's' : ''} Need Review</p>
              <p className="text-red-500 text-xs">Tap to review and take action</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-2.5">
          {cards.map(({ href, label, desc, icon: Icon, color }) => (
            <Link key={href} href={href}
              className="bg-white border border-[#e8e8e8] rounded-xl p-4 flex items-center gap-3 hover:border-[#1a6b3a]/30 transition-all group">
