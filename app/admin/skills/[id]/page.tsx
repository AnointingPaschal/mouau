...
import {
  ...
  Lightbulb, Star, AlertTriangle, Quote, Calendar, Users, MapPin, Clock
} from 'lucide-react'
...
export default function AdminSkillDetailPage() {
  ...
  // New class form
  const [classForm, setClassForm] = useState({ title:'', description:'', date:'', location:'', duration_hours:'', max_attendees:'', notify_students: true })
  ...
  const saveSkill = async () => {
    setSaving(true)
    await fetch(`/api/admin/skills/${id}`, { method:'PUT', headers:{'Content-Type':'application/json',...authH}, body: JSON.stringify(skill) })
    setSaving(false)
  ...
  const uploadImage = async (file: File) => {
    ...
    setUploading(false)
    showToast(d.url ? 'Image uploaded!' : 'Upload failed')
  }
  ...
  const announceClass = async () => {
    ...
    if (d.ok) { setClasses(p => [d.data, ...p]); setClassForm({ title:'', description:'', date:'', location:'', duration_hours:'', max_attendees:'', notify_students:true }) }
    setAddingClass(false)
    showToast(classForm.notify_students ? 'Class announced & students notified!' : 'Class announced!')
  }
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex items-center gap-3 mb-4">
              <button onClick={() => router.push('/admin/skills')} className="w-8 h-8 rounded-xl bg-[#f0f0f0] flex items-center justify-center">
              ...
              {tab === 'edit' && (
                <button onClick={saveSkill} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 bg-[#1a6b3a] text-white rounded-xl text-sm font-bold disabled:opacity-50">
                ...
                <div className="card overflow-hidden">
                  <div className="relative h-36 bg-[#f0f0f0] overflow-hidden">
                    ...
                    <button onClick={() => fileRef.current?.click()} disabled={uploading}
                      className="absolute bottom-2 right-2 flex items-center gap-1.5 bg-white/90 text-xs font-semibold px-3 py-1.5 rounded-lg shadow">
              ...
              <div className="space-y-3">
                <div className="card p-4 space-y-3 border-2 border-dashed border-[#e8e8e8]">
                  ...
                  <button onClick={addNugget} disabled={addingNugget || !nuggetForm.content.trim()}
                    ...
                    {addingNugget ? <Loader2 className="w-3.5 h-3.5 animate-spin"/> : <Plus className="w-3.5 h-3.5"/>}
                    Add Nugget
                  </button>
                </div>
                ...
                {nuggets.map((n, i) => {
                  ...
                  return (
                    <div key={n.id} className="card p-3.5 flex items-start gap-3">
                      ...
                      <button onClick={() => deleteNugget(n.id)} className="text-[#ddd] hover:text-red-400 flex-shrink-0 transition-colors">
                ...
                {nuggets.length === 0 && (
                  <div className="text-center py-8 text-[#aaa] text-sm">No nuggets yet. Add your first one above.</div>
                )}
              </div>
            )}
              ...
              <div className="space-y-3">
                <div className="card p-4 space-y-3 border-2 border-dashed border-[#e8e8e8]">
                  ...
                  <div className="flex items-center justify-between py-1">
                    <div>
                      ...
                      <p className="text-[10px] text-[#aaa]">{approvedCount} approved student{approvedCount !== 1 ? 's' : ''}</p>
                    </div>
                  ...
                  <button onClick={announceClass} disabled={addingClass || !classForm.title.trim()}
                    ...
                    {addingClass ? <Loader2 className="w-3.5 h-3.5 animate-spin"/> : <Calendar className="w-3.5 h-3.5"/>}
                    Announce Class
                  </button>
                </div>
                ...
                {classes.map(cls => (
                  <div key={cls.id} className="card p-3.5">
                    <p className="font-bold text-sm text-[#0a0a0a] mb-1">{cls.title}</p>
                    {cls.date && <p className="text-[11px] text-[#6b6b6b] flex items-center gap-1.5 mb-0.5"><Calendar className="w-3 h-3 text-[#1a6b3a]"/>{new Date(cls.date).toLocaleString()}</p>}
                    {cls.location && <p className="text-[11px] text-[#6b6b6b] flex items-center gap-1.5"><MapPin className="w-3 h-3 text-[#1a6b3a]"/>{cls.location}</p>}
                  </div>
                ))}
                {classes.length === 0 && <div className="text-center py-8 text-[#aaa] text-sm">No classes announced yet.</div>}
              </div>
            )}
              ...
              <div className="space-y-2">
                <div className="grid grid-cols-3 gap-2 mb-3">
                  <div className="card p-3 text-center">
                    <p className="font-black text-lg text-amber-500">{pendingCount}</p>
                    <p className="text-[9px] text-[#aaa] font-semibold uppercase">Pending</p>
                  </div>
                  <div className="card p-3 text-center">
                    <p className="font-black text-lg text-[#1a6b3a]">{approvedCount}</p>
                    <p className="text-[9px] text-[#aaa] font-semibold uppercase">Approved</p>
                  </div>
                  <div className="card p-3 text-center">
                    <p className="font-black text-lg text-[#0a0a0a]">{apps.length}</p>
                    <p className="text-[9px] text-[#aaa] font-semibold uppercase">Total</p>
                  </div>
                </div>

                {apps.length === 0 ? (
                  <div className="text-center py-8 text-[#aaa] text-sm"><Users className="w-8 h-8 text-[#ddd] mx-auto mb-2"/>No applications yet.</div>
                ) : apps.map((app: any) => (
                  <div key={app.id} className="card p-3.5">
                    ...
                    {app.status === 'pending' && (
                      <div className="flex gap-2 mt-2">
                        <button onClick={() => updateAppStatus(app.id, 'rejected', app.student_id)}
                          className="flex-1 py-1.5 text-[11px] font-bold text-red-500 border border-red-200 rounded-xl hover:bg-red-50 transition-colors">
                          Reject
                        </button>
                        <button onClick={() => updateAppStatus(app.id, 'approved', app.student_id)}
                          className="flex-1 py-1.5 text-[11px] font-bold text-white bg-[#1a6b3a] rounded-xl hover:bg-[#145530] transition-colors">
                          Approve
                        </button>
                      </div>
                    )}
                    {app.status !== 'pending' && (
                      <button onClick={() => updateAppStatus(app.id, 'pending', app.student_id)}
                        className="mt-2 text-[10px] text-[#aaa] hover:text-[#6b6b6b] underline">Reset to pending</button>
                    )}
                  </div>
                ))}
              </div>
